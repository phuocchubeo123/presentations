\begin{frame}
    ...
    \only<1>{
    \begin{itemize}
        \item Bob can send all $u^d$ prefixes to Alice.
...
\begin{frame}
    ...
    \only<2>{\frametitle{Trade-off in Tree Search - Better Pruning}}
    \begin{itemize}
        \only<1>{\item The previous slide is a bit misleading!}
        \only<2>{\item Bob can slice differently to help Alice with more efficient search.}
    \end{itemize}
    \centering
    \begin{tikzpicture}[grow=down,level distance=0.9cm,
        base/.style={circle,draw,minimum size=5pt,inner sep=1.2pt},
        ...
        % Annotation to highlight the costly path
        \node[below right=3pt and 8pt of b6,shape=rectangle,draw=none,fill=none,inner sep=1pt,outer sep=0pt] (b6note) {$\text{takes }2^u\text{ fails to find out!}$};
        \draw[->,thick,shorten >=1pt] (b6note.west) -- (b6);
    \end{tikzpicture}
\end{frame}
